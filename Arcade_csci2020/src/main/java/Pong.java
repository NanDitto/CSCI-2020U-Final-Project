...
public class Pong extends Application {
	...
	/*
	 * Ball Properties: X and Y Speed Size of the ball is a circle
	 */
	float xspeed = 0;
	float yspeed = -4;
	...
	/*
	 * Paddle Properties: Y Speed Size of Paddle is a Rectangle Has a length and
	 * width Only moves with a key press
	 */
	...
	public void start(Stage stage) {
		...
		Timeline Movement = new Timeline(new KeyFrame(Duration.millis(20), new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent t) {
				...
				if (ball.getLayoutX() <= Paddle1.getX() + W
						&& ((ball.getLayoutY() >= Paddle1.getY()) && (ball.getLayoutY() <= Paddle1.getY() + LENGTH))) {
					xspeed -= 3;
					xspeed = -xspeed;
				}
				// If the ball touches right paddle
				if (ball.getLayoutX() >= Paddle2.getX() - W
						&& ((ball.getLayoutY() >= Paddle2.getY()) && (ball.getLayoutY() <= Paddle2.getY() + LENGTH))) {
					xspeed += 3;
					xspeed *= -1;
				}
				// If the ball reaches the left Player 2 gets a Point and reset
				// ball
				// If the ball reaches the right Player 1 gets a point and reset
				// ball
				...
				// If the ball reaches the bottom or top border change the
				// direction
				if ((ball.getLayoutY() >= (HEIGHT - ball.getRadius()))
