...
public class Jeopardy extends Application {
	...
	int one = 4, two = 4, three = 4, four = 4, five = 4; // counter for each row
															// and column
	...
	ArrayList<String> tokens = new ArrayList<String>(); // used to store
														// cateogory from
														// arraylist
	ArrayList<String> questions[] = new ArrayList[21]; // used to store question
														// from arraylist
	ArrayList<String> answers[] = new ArrayList[21]; // used to store answers
														// from arraylist
	...
	public void start(Stage primaryStage) throws Exception {
		...
		Window = primaryStage;
		layout();
		ImageView title = new ImageView("file:" + cwd + "/src/main/resources/title.gif"); // title
																							// on
																							// top
		title.setFitHeight(150);
		title.setFitWidth(960);
		ImageView main = new ImageView("file:" + cwd + "/src/main/resources/back.png"); // background
																						// image
		main.setFitHeight(600);
		main.setFitWidth(960);
		showLines(cwd + "/src/main/resources/questionAnswers.txt"); // for
																	// getting
																	// random
																	// category
																	// with
		...
		for (int i = 1; i < numButtons.length; i++) {
			for (int j = 1; j < numButtons.length; j++) {
				numButtons[i][j].setOnAction(this::handleButtonAction); // handling
																		// for
																		// all
																		// buttons
			}
		...
		numbers.setPadding(new Insets(10, 5, 10, 5)); // margins around the
														// whole grid
		...
		scene.getStylesheets().add("file:" + cwd + "/src/main/resources/style.css"); // used
																						// to
																						// design
																						// the
																						// buttons
	...
	public void layout() { // to make a 5 by 5 with name acccording to the row
							// and column
		for (int i = 1; i <= 5; i++) {
			for (int p = 1; p <= 5; p++) {
				if (i == 1) {
	...
	public void showLines(String fileName) { // read the category, quesitions
												// and answers from the file and
...
>>>>>>> 92bbedf29cee1d5fb3f4176a18fdc745f7f88896
	...
	private void handleButtonAction(ActionEvent event) { // used
		...
		if (mainCounter == 25) { // if all the boxes are pressed on, then output
									// the dialog
			if (score >= 2000) {
				JOptionPane.showMessageDialog(null, "You won! You money will be tranfered to you in 1 decade.",
						"Game Over!", JOptionPane.INFORMATION_MESSAGE);
				WIN = true;
			}
			if (score <= 2000) {
				JOptionPane.showMessageDialog(null, "You lost! Try again to unlock the next level", "Game Over!",
	...
	private void action(Button x, int number, int rando, int points) { // do
																		// various
																		// things
		Random rand = new Random();
		...
		if (entered.toUpperCase().contains(answers[number].get(temp).toUpperCase())) { // checking
																						// userinput
																						// and
																						// answer
			score = score + points;
		...
		questions[number].remove(temp); // removing elements from arrays to
										// avoid duplication
		fadeAndDisable(x); // fading the button and disabling it

	}

	private void fadeAndDisable(Button x) { // fade buttons when they are
											// pressed
